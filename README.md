...
```bash
go run . --init-config --config ./mindgit.json
go run . --set-password --config ./mindgit.json
```

Run from this repository:

```bash
go run . --config ./mindgit.json
```
...
```json
{
  ...
  "ssh": {
    "dataDir": "mindgit-data",
    "knownHosts": "mindgit-data/known_hosts",
...
```bash
mindgit --config ./mindgit.json --import-ssh-key ~/.ssh/bastion_ed25519 --key-name bastion
mindgit --config ./mindgit.json --import-ssh-key ~/.ssh/production_ed25519 --key-name production
```

The imported files are AES-GCM encrypted under
`mindgit-data/keys/<name>.key.enc`. The encryption key is derived from the
...
remote shell in `remoteDir`. MindGit keeps a separate `known_hosts` file, accepts
